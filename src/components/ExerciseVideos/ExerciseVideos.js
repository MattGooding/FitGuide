...
const ExerciseVideos = ({ exerciseVideos, name }) => {
    ...
    return (
        <Box className="exerciseVideos" sx={{ marginTop: { lg: '200px', xs: '20px' } }} p="20px">
            ...
            <Stack justifyContent="flex-start" flexWrap="wrap" alignItems="center" sx={{ flexDirection: { lg: 'row' }, gap: { lg: '110px', xs: '0px' } }}>
                {exerciseVideos?.slice(0, 3).map((item, index) => (
                    <a key={index} className='exerciseVideo' href={`https://www.youtube.com/watch?v=${item.video.videoId}`} target='_blank' rel='noreferrer'>
